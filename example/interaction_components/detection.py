...
def hydrophobic_interactions(atom_set_a, atom_set_b):
    ...
    data = namedtuple(
        'hydroph_interaction',
        'bsatom bsatom_orig_idx bsatom_coords ligatom ligatom_orig_idx ligatom_coords sidechain '
    ...
    for a, b in itertools.product(atom_set_a, atom_set_b):
        ...
        contact = data(
            ...
            bsatom_coords=a.coords,
            ligatom=b.atom,
            ligatom_orig_idx=b.orig_idx,
            ligatom_coords=b.coords,
...
def hbonds(acceptors, donor_pairs, protisdon, typ):
    ...
    data = namedtuple(
        'hbond',
        'a a_orig_idx d a_coords d_orig_idx d_coords h h_orig_idx h_coords distance_ah distance_ad angle type protisdon resnr '
    ...
    for acc, don in itertools.product(acceptors, donor_pairs):
        ...
        contact = data(
            ...
            d_coords=don.d_coords,
            h=don.h,
            h_orig_idx=don.h_orig_idx,
            h_coords=don.h_coords,
            distance_ah=dist_ah,
            distance_ad=dist_ad,
            angle=v,
            type=typ,
...
def pistacking(rings_bs, rings_lig):
    ...
    data = namedtuple(
        ...
        'ligatoms_coords distance angle offset type restype resnr reschain restype_l resnr_l reschain_l')
    pairings = []
    for r, l in itertools.product(rings_bs, rings_lig):
        ...
        if passed:
            contact = data(
                proteinring=r,
                ligandring=l,
                bsatoms_orig_idx=r.atoms_orig_idx,
                ligatoms_orig_idx=l.atoms_orig_idx,
                bsatoms_coords=r.atoms_coords,
                ligatoms_coords=l.atoms_coords,
                distance=d,
                angle=a,
                offset=offset,
                type=ptype,
...
def pication(rings, pos_charged, protcharged):
    ...
    data = namedtuple(
        'pication',
        'ring ring_orig_idx ring_atoms_coords charge charge_orig_idx charge_atoms_coords '
        'distance offset type restype resnr reschain restype_l resnr_l reschain_l protcharged')
    pairings = []
        ...
        for p in pos_charged:
            ...
            if type(p).__name__ == 'lcharge' and p.fgroup == 'tertamine':
                ...
                if not a > 30.0:
                    ...
                    contact = data(
                        ring=ring,
                        ring_orig_idx=ring.atoms_orig_idx,
                        ring_atoms_coords=ring.atoms_coords,
                        charge=p,
                        charge_orig_idx=p.atoms_orig_idx,
                        charge_atoms_coords=p.atoms_coords,
                        distance=d,
                        offset=offset,
                        type='regular',
                        restype=restype,
...
def saltbridge(poscenter, negcenter, protispos):
    ...
    data = namedtuple(
        'saltbridge',
        'positive pos_orig_idx pos_atoms_coords negative neg_orig_idx neg_atoms_coords '
        'distance protispos resnr restype reschain resnr_l restype_l reschain_l')
    pairings = []
    for pc, nc in itertools.product(poscenter, negcenter):
        ...
        contact = data(
            positive=pc,
            pos_orig_idx=pc.atoms_orig_idx,
            pos_atoms_coords=pc.atoms_coords,         
            negative=nc,
            neg_orig_idx=nc.atoms_orig_idx,
            neg_atoms_coords=nc.atoms_coords, 
...
def halogen(acceptor, donor):
    ...
    data = namedtuple(
        'halogenbond',
        'acc acc_orig_idx acc_coords don don_orig_idx don_coords distance don_angle acc_angle restype '
    ...
    for acc, don in itertools.product(acceptor, donor):
        ...
        contact = data(
            ...
            acc_coords=acc.o_coords,
            don=don,
            don_orig_idx=don.x_orig_idx,
            don_coords=don.x_coords,
            distance=dist,
            don_angle=don_angle,
            acc_angle=acc_angle,
            restype=restype,
...
def water_bridges(bs_hba, lig_hba, bs_hbd, lig_hbd, water_ligand, water_protein):
    """Find water-bridged hydrogen bonds between ligand and protein. For now only considers bridged of first degree."""
    data = namedtuple(
        'waterbridge', 'a a_orig_idx a_coords atype d d_orig_idx d_coords dtype '
        'h h_orig_idx h_coords water water_orig_idx water_coords water_type distance_aw '
    ...
    for w in water_ligand + water_protein:
...
def metal_complexation(metals, metal_binding_lig, metal_binding_bs):
    ...
    data = namedtuple(
        'metal_complex',
        'metal metal_orig_idx metal_type target target_orig_idx target_coords target_type '
