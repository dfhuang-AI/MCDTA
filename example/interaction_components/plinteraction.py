...
class Mol:
    ...
    def find_rings(self):
        ...
        data = namedtuple(
            'aromatic_ring',
            'atoms orig_atoms atoms_orig_idx atoms_coords normal obj center type')
        rings = []
        ...
        for ring in rings_atom_idx:
            ...
            if 4 < len(ring) <= 6:
                ...
                if is_aromatic(r_atoms) or res[0] in aromatic_amino or ring_is_planar(
                    ...
                    atoms_coords = [get_atom_coords(r_atom) for r_atom in r_atoms]
                    orig_atoms = r_atoms
                    rings.append(
                        data(
                            atoms=r_atoms,
...
# =============================================================================
#### 改进后：直接判断配体中是否含有水分子，不用输入水分子
        self.water = []
        #if mol_water is not None:
            #self.mol_water = mol_water
            #self.mol_water_conf = mol_water.GetConformers()[0]
        data = namedtuple('water', 'oxy oxy_orig_idx oxy_coords')
        for hoh in self.all_atoms:
            oxy = None
            if hoh.GetSymbol() == 'O' and hoh.GetTotalNumHs() == 2 and hoh.GetAtomicNum() == 8:
                #print('H2O')
    ...
    def find_hbd(self):
        ...
        data = namedtuple(
            'hbonddonor',
            'd d_orig_atom d_orig_idx h h_orig_idx type d_coords h_coords')
    ...
    def find_charged(self):
        ...
        data = namedtuple(
            'lcharge',
            'atoms orig_atoms atoms_orig_idx atoms_coords type center fgroup')
        a_set = []
        for a in self.mol.GetAtoms():
                ...
                a_set.append(
                    data(
                        atoms=[
                            ...
                            a_orig_idx, ], atoms_coords = [get_atom_coords(a),],
                                type='positive', center=list(
                            get_coord(
            ...
            if self.is_functional_group(a, 'phosphate'):###包含相邻原子的贡献
                a_contributing = [a, ]
                ...
                a_set.append(
                    data(
                        atoms=list(set(a_contributing)),
                        orig_atoms=list(set(orig_contributing)),
                        atoms_orig_idx=list(set(a_contributing_orig_idx)),
                        atoms_coords=list(set([get_atom_coords(atom) for atom in a_contributing])),
                        type='negative',
                        center=list(
                            get_coord(
                                self.mol_conf,
                                a.GetIdx())),
                        fgroup='phosphate'))
            if self.is_functional_group(a, 'sulfonicacid'):###包含相邻氧原子的贡献
                a_contributing = [a, ]
                a_contributing_orig_idx = [a_orig_idx, ]
                       ...
                       atoms_coords=list(set([get_atom_coords(atom) for atom in a_contributing])),
                        ...
                        center=list(
                            get_coord(
                                ...
                                a.GetIdx())),
                        fgroup='sulfonicacid'))
            elif self.is_functional_group(a, 'sulfate'):###包含相邻原子的贡献
                a_contributing = [a, ]
                a_contributing_orig_idx = [a_orig_idx, ]
                [a_contributing.append(neighbor)
                 for neighbor in a.GetNeighbors()]
                ...
                a_set.append(
                    data(
                        ...
                        center=get_coord(
                            ...
                            a.GetIdx()),
                        fgroup='sulfate'))
            if self.is_functional_group(a, 'carboxylate'):###包含相邻氧原子的贡献
                a_contributing = [
                ...
                a_set.append(data(atoms=a_contributing,
                                  ...
                                  atoms_coords=[get_atom_coords(atom) for atom in a_contributing],
                                  type='negative',
                                  center=centroid([get_coord(self.mol_conf,
                                                             a.GetIdx()) for a in a_contributing]),
                                  fgroup='carboxylate'))
            elif self.is_functional_group(a, 'guanidine'):###包含相邻N原子的贡献
                a_contributing = [
...
class Protein(Mol):
    ...
    def find_charged(self):
        ...
        data = namedtuple(
            'pcharge',
            'atoms atoms_orig_idx atoms_coords type center restype resnr reschain')
        a_set = []
...
class PLInteraction:
    ...
    def __init__(self, lig_obj, bs_obj, pdbid):
        ...
        self.water_ligand = self.ligand.water
        self.water_protein = self.protein.water
        ...
        self.pication_paro = pication(
            ...
            self.ligand.get_pos_charged(),
            False)
        #对于包含组氨酸的星座来说很重要。如果组氨酸环与配体中的芳环处于堆叠位置，
        #则由于组氨酸在环中也携带正电荷，因此在大多数情况下存在pi-pi堆积和阳离子相互作用。对于这种情况，只报告pi-pi堆积
        ...
        #1. 通过pi-pi堆叠相互作用的环之间不能有额外的疏水接触
        #2. 如果一个配体原子与同一残基中的几个结合位点原子相互作用，则只保留距离最近的一个
        #3. 如果一个蛋白质原子与几个相邻的配体原子相互作用，只保留距离最近的那个
        ...
        self.all_water_bridges = water_bridges(
            ...
            self.ligand.get_hbd(),
            self.water_ligand,self.water_protein)
        #已经形成氢键的供体原子不允许形成水桥。
